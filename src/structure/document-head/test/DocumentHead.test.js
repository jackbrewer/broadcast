...
describe('Component: DocumentHead', function () {
  ...
  it.skip('should output expected elements', function (done) {
    ReactDOM.render(
      <Router>
        <DocumentHead />
      </Router>,
    ...
    setImmediate(() => {
      assert.equal(document.title, 'Picks Backline')
      assert.equal(document.querySelector('link[rel="canonical"]').getAttribute('href'), 'https://picks.tech/')
      done()
    })
  })

  it.skip('should allow child child components to extend title using the template', function (done) {
    ReactDOM.render(
      <Router>
        <div>
          <DocumentHead />
    ...
    setImmediate(() => {
      assert.equal(document.title, 'Example | Picks Backline')
      done()
    })
  })
})
